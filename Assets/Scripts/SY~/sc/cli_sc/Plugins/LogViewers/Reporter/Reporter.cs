...
{
    //自添加
    public int showScreenWidth = 1280;
    public int showScreenHeight = 720;
    public Vector2 logBtnSize = Vector2.zero;
    private bool calLogBtnSize = false;
	...
	void addSample()
	{
		...
		sample.fps = fps;
		sample.fpsText = fps.ToString("0.000"); ;
#if UNITY_CHANGE3
	...
	void drawToolBar()
	{
        ...
        if (GUILayout.Button(searchContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            if (selectedLog != null)
            {
#if UNITY_STANDALONE
                TextEditor textEditor = new TextEditor();
                textEditor.text = selectedLog.condition;
                textEditor.SelectAll();
                textEditor.Copy();
#elif UNITY_ANDROID

                AndroidJavaClass util = new AndroidJavaClass("com.shiyuegame.fswy.ClipboardUtil");

                AndroidJavaClass jc = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
                AndroidJavaObject activty = jc.GetStatic<AndroidJavaObject>("currentActivity");

                util.CallStatic("copyTextToClipboard", activty, selectedLog.stacktrace);
#elif UNITY_IPHONE  && !UNITY_EDITOR
                 _CopyToClipboard(selectedLog.stacktrace);
#endif
            }
        }
...
#if UNITY_STANDALONE && !UNITY_EDITOR
            RECT rect = new RECT();
            GetWindowRect(GetActiveWindow(), ref rect);
...
#if UNITY_STANDALONE && !UNITY_EDITOR
    IEnumerator SetWindowPos()
    {
        yield return 1;
        int posX = PlayerPrefs.GetInt("window_x", -1);
        int posY = PlayerPrefs.GetInt("window_y", -1);
        if (posX != -1 && posY != -1)
        {
            SetWindowPos(GetActiveWindow(), 0, posX, posY, 0, 0, 0x001);
        }
        yield return null;
    }
#endif
	...
	{
        if(!calLogBtnSize && Screen.width != 0 && Screen.width < Screen.height)
        {
			logBtnSize.x  = Screen.width * 0.15f;
            logBtnSize.y = logBtnSize.x * 0.5f;

			float closeBtnSize = Screen.height * (32.0f / 1280.0f);
            size.x = size.y = closeBtnSize;
            initializeStyle();

            calLogBtnSize = true;
        }

        if (!show && GUILayout.Button("日志", GUILayout.Width(logBtnSize.x), GUILayout.Height(logBtnSize.y)))
        {
            ...
            Screen.SetResolution(showScreenWidth, showScreenHeight, false);
            StartCoroutine(SetWindowPos());
#endif
            show = true;
        ...
        if (!show || clickFrame == Time.frameCount)
        {
			return;
		}
	...
	bool isGestureDone()
	{
		...
		if (Application.platform == RuntimePlatform.Android ||
            ...
            {
                baseX = Input.touches[0].position.x;
                checkState = true;
            }
            if ((Input.touches[0].phase == TouchPhase.Canceled || Input.touches[0].phase == TouchPhase.Ended) && checkState)
            {
                float newX = Input.touches[0].position.x;
                float deltaX = Mathf.Abs(newX - baseX);
                baseX = 0.0f;
                checkState = false;
                return deltaX >= 1000;
            }
            ...
            {
                baseX = Input.mousePosition.x;
                checkState = true;
            }

            if (Input.GetMouseButtonUp(0) && checkState)
            {
