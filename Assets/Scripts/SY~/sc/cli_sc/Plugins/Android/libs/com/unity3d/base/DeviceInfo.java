...
import com.shiyuegame.fswy.XPlanUtils;
import com.shiyuegame.fswy.emulator.EmulatorCheckUtil;
...
public class DeviceInfo {
   ...
   * {@code /sys/devices/system/cpu/possible},
   * {@code /sys/devices/system/cpu/present},
   * then {@code /sys/devices/system/cpu/}.
   *
   * @return Number of CPU cores in the phone, or DEVICEINFO_UKNOWN = -1 in the
   *         event of an error.
   */
  public static int getNumberOfCPUCores() {
    if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.GINGERBREAD_MR1) {
      ...
      // but a
      // handful of devices (Atrix 4G and Droid X2 for example) were released with a
      // dual-core
      // chipset and Gingerbread; that can let an app in the background run without
      // impacting
  ...
  /**
   * Tries to read file contents from the file location to determine the number of
   * cores on device.
   * 
   ...
   * the next available number after the match in the file being read and returns
   * it as an integer.
   * 
   ...
   *         Returns
   *         DEVICEINFO_UNKNOWN = -1 in the event that no more numbers exist on
   *         the same line.
   */
  ...
  public boolean isEmulator() {
    if (XPlanUtils.notHasBlueTooth())
      return true;
    if (XPlanUtils.notHasLightSensorManager(this.context))
      return true;
    if (XPlanUtils.isFeatures())
      return true;
    if (XPlanUtils.checkIsNotRealPhone())
      return true;
    return com.shiyuegame.fswy.emulator.EmulatorCheckUtil.getSingleInstance().readSysProperty(this.context, null);
  }

  public String checkEmulator() {
    StringBuilder sb = new StringBuilder();
    sb.append("没有蓝牙：" + XPlanUtils.notHasBlueTooth());
    sb.append("\n");
    sb.append("没有光传感器：" + XPlanUtils.notHasLightSensorManager(this.context));
    sb.append("\n");
    sb.append("有其它特征：" + XPlanUtils.isFeatures());
    sb.append("\n");
    sb.append("不是真的手机：" + XPlanUtils.checkIsNotRealPhone());
    sb.append("\n");

    sb.append("EmulatorCheckUtil检查是模拟器:" + EmulatorCheckUtil.getSingleInstance().readSysProperty(this.context, null));
    sb.append("\n");
