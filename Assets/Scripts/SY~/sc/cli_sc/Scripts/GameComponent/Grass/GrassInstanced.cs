...
public class GrassInstanced : MonoBehaviour
{
    ...
    public static int instanceCount = 100;
    ...
    private void Awake()
    {
        ...
        GrassManager.camera = GameContext.GetInstance().MainCamera;
        GrassManager.cameraTransform = GameContext.GetInstance().MainCamera.transform;
    ...
    void OnDestroy()
    {
        ...
        {
            if(GameContext.GetInstance().MainCamera == null)
                return;
            GameContext.GetInstance().MainCamera.RemoveCommandBuffer(CameraEvent.AfterForwardOpaque, commandBuffer);
        }
    }
    ...
    private void SummitCommandBuffer()
    {
        if (commandBuffer != null)
        {
            return;
            ...
            for (int i = 0; i < allList.Count; i++)
            {
                ...
                if (grass.mat_id == ii)
                {
                    ...
                    if (materixList.Count >= instanceCount)
                    {
                        ...
                        commandBuffer.DrawMeshInstanced(instanceMesh, 0, matList[ii], 0, materixList.ToArray(), materixList.Count, materialPropertyBlock);
        ...
        GameContext.GetInstance().MainCamera.AddCommandBuffer(CameraEvent.AfterForwardOpaque, commandBuffer);
    ...
    void Update()
    {
        ...
        if (Application.isPlaying)
        {
            GrassManager.Instance.CheckGrassInView();
