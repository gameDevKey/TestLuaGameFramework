...
namespace EditorTools.AssetBundle {
    /// <summary>
    /// prefab,material,font,animatorController等类型资源的处理器基类
    /// </summary>
    public class AssetProcessor {
        public const string ASSET_PREFAB = ".prefab";
        ...
        /// 3.移除pathList中和之前节点筛选结果相同的部分RemoveFilteredPath(filteredPathSet, pathList);
        /// 4.在dependAssetPathList中移除pathList，RemoveFromDependPathList(dependAssetPathList, pathList);
        /// 5.将pathList加入筛选资源集合AddToFilteredPathSet(filteredPathSet, pathList);
        ...
        protected virtual void SaveAsset(Object asset, string path) {
        ...
        /// <returns></returns>
        protected virtual HashSet<string> ApplyStrategyNode(string path, Object asset, StrategyNode node) {
            return new HashSet<string>();
        }

        protected HashSet<string> ApplyEmptyStrategyNode(List<string> dependPathList, StrategyNode node) {
        ...
        public void RemoveFilteredPath(HashSet<string> filteredPathSet, List<string> pathList) {
        ...
        public void AddToFilteredPathSet(HashSet<string> filteredPathSet, List<string> pathList) {
            foreach (string path in pathList) {
                filteredPathSet.Add(path);
            }
        }

        public void ReplaceWithTempAssetPath(List<string> pathList) {
            for (int i = 0; i < pathList.Count; i++) {
                if (TemporaryAssetHelper.HasCreatedTempAsset(pathList[i]) == true) {
                    pathList[i] = TemporaryAssetHelper.GetTempAssetPath(pathList[i]);
                }
            }
        }

        public void RemoveFromDependPathList(List<string> dependPathList, List<string> pathList) {
            foreach (string path in pathList) {
                dependPathList.Remove(path);
            }
        }
