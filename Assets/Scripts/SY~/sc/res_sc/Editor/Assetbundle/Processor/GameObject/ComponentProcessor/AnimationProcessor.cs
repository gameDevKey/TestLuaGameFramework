...
namespace EditorTools.AssetBundle {
    public class AnimationProcessor : ComponentProcessor {
        ...
        public override HashSet<string> Process(string entryPath, GameObject go, StrategyNode node) {
            ...
            if (animation != null) {
                ...
                if (defaultClip != null) {
                    ...
                    if (node.pattern.IsMatch(defaultClipPath) == true) {
                ...
                for (int i = 0; i < clips.Length; i++) {
                    ...
                    if (clip != null) {
                        ...
                        if (node.pattern.IsMatch(clipPath) == true) {
        ...
        //clipTokens记录AnimationClip的key，其中第一个值为默认clip的key值
        ...
        private void VerifyClipsName(string entryPath, AnimationClip[] clips) {
            ...
            for (int i = 0; i < clips.Length; i++) {
                if (clips[i] != null) {
                    if (set.Contains(clips[i].name) == true) {
                        string msg = entryPath + "  Animation 中存在同名AnimationClip，需要对相关AnimationClip重命名！";
                        AssetBundleExporter.ThrowException(msg);
                    }
                    set.Add(clips[i].name);
                }
            }
        }
    }
