    ...
    public class AtlasGenerator {
        ...
        public static string[] Generate(string[] folderPaths) {
            ...
            for (int i = 0; i < folderPaths.Length; i++) {
                ...
                if(!folderPath.StartsWith(UIPrefabProcessor.UI_TEXTURE_ROOT))
                {
                    continue;
                }
        ...
        public static string GetAtlasPath(string path) {
            string folderPath = UIPrefabProcessor.GetFolderPath(path);
            string tag = GetTexturePackingTag(path);
            return GetAtlasPath(folderPath, tag);
        }
        ...
        public static string GetMaterialPath(string folderPath, string tag) {
            return GetAtlasPath(folderPath, tag).Replace(".png", ".mat");
        }
        ...
        public static string GetTexturePackingTag(string path) {
            TextureImporter importer = AssetImporter.GetAtPath(path) as TextureImporter;
            if (importer == null) 
            {
                return null;
                //Debug.LogError("GetTexturePackingTag:imprter is null[" + path + "]");
            }
            else
            {
                string path2 = path.Substring(0, path.LastIndexOf('/'));
                string tag = path2.Substring(path2.LastIndexOf('/') + 1);
                return tag;
            }
        }
		...
		private static Texture2D CreateAtlas(string atlasPath, TextureData[] textureDatas) {
            ...
            int impType = TextureImporterUtil.SPLIT_TEXTURE;
            TextureImporterUtil.CreateMultipleSpriteImporter(atlasPath, rects, GetPackTextureNames(textureDatas), GetSpriteBorders(textureDatas),
            atlasWidth, atlasHeight, ATLAS_MAX_SIZE, impType);

            AssetDatabase.ImportAsset(atlasPath, ImportAssetOptions.ForceUpdate);
            MaterialCreator.Create(atlasPath);
        ...
        private static string[] GetPackTextureNames(TextureData[] textureDatas) {
