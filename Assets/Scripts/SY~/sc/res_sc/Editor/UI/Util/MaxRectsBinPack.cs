...
namespace EditorTools.UI {
    public class Texture2DComparison : IComparer<TextureData> {
        public int Compare(TextureData x, TextureData y) {
            int ret = 0;
            if (Mathf.Max(x.width, x.height) > Mathf.Max(y.width, y.height)) {
                ret = -1;
            } else if (Mathf.Max(x.width, x.height) < Mathf.Max(y.width, y.height)) {
                ret = 1;
            }
            return ret;
        }
    ...
    public class MaxRectsBinPack {
        ...
        public static Rect[] PackTextures(Texture2D texture, Texture2D[] textures) {
            int maxWidth = LogPow(textures[0].width);
            int maxHeight = LogPow(textures[0].height);
