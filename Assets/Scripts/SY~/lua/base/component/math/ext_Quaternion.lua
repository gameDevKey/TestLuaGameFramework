local get = xlua.getmetatable(CS.UnityEngine.Quaternion)
...
function Quaternion.AngleAxisToQuat(angle,axis,quat)
	local normAxis = axis:Normalize()
    angle = angle * halfDegToRad
    local s = sin(angle)    
...
Quaternion.MulToQuat = function(lhs,rhs,quat)
	quat:Set((((lhs.w * rhs.x) + (lhs.x * rhs.w)) + (lhs.y * rhs.z)) - (lhs.z * rhs.y), (((lhs.w * rhs.y) + (lhs.y * rhs.w)) + (lhs.z * rhs.x)) - (lhs.x * rhs.z), (((lhs.w * rhs.z) + (lhs.z * rhs.w)) + (lhs.x * rhs.y)) - (lhs.y * rhs.x), (((lhs.w * rhs.w) - (lhs.x * rhs.x)) - (lhs.y * rhs.y)) - (lhs.z * rhs.z))	
end
...
__extends.identity = function() return _new(0, 0, 0, 1) end
__extends.eulerAngles = Quaternion.ToEulerAngles
