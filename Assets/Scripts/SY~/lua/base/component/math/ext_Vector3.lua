local get = xlua.getmetatable(CS.UnityEngine.Vector3)
...
function Vector3:SetByVector3(v)
	self.x = v.x
	self.y = v.y
	self.z = v.z 
end
...
function Vector3.Pop(x,y,z)
	local v = PoolManager.Instance:Pop(PoolType.class,PoolDefine.poolKey.vector3)
...
function Vector3.PopByVector3(data)
	local v = PoolManager.Instance:Pop(PoolType.class,PoolDefine.poolKey.vector3)

	if v then
		v:Set(data.x,data.y,data.z)
	else
		v = Vector3(data.x,data.y,data.z)
	end

	return v
end

function Vector3.Push(v)
	PoolManager.Instance:Push(PoolType.class,PoolDefine.poolKey.vector3,v)
end

function Vector3:OnReset()
	self.x = 0
	self.y = 0
	self.z = 0
end
...
function Vector3.MulNumber(va,q,v)
	v.x = va.x * q
	v.y = va.y * q
	v.z = va.z * q
	return v
...
function Vector3.AddTo(va,vb,v)
	v:Set(va.x + vb.x, va.y + vb.y, va.z + vb.z)
	return v
end
...
function Vector3.SubTo(va,vb,v)
	v:Set(va.x - vb.x, va.y - vb.y, va.z - vb.z)
	return v
end
...
__extends.up 		= function() return _new(0,1,0) end
__extends.down 	= function() return _new(0,-1,0) end
__extends.right	= function() return _new(1,0,0) end
__extends.left	= function() return _new(-1,0,0) end
__extends.forward = function() return _new(0,0,1) end
__extends.back	= function() return _new(0,0,-1) end
__extends.zero	= function() return _new(0,0,0) end
__extends.one		= function() return _new(1,1,1) end

__extends.magnitude = Vector3.Magnitude
__extends.normalized    = Vector3.Normalize
__extends.sqrMagnitude = Vector3.SqrMagnitude
