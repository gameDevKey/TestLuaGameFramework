BattleSearchSystem = BaseClass("BattleSearchSystem",SECBSystem)

function BattleSearchSystem:__Init()
    local _ = SearchSorter
    self:InitSort()
...
function BattleSearchSystem:__Delete()

end

function BattleSearchSystem:SearchByRange(param,range)
...
function BattleSearchSystem:OnLateUpdate()
    --Log("迭代次数",self.world.EntitySystem.entityList.length,self.searchByRangeNum,self.num,self.debugTime)
    --Log("迭代时间",DEBUG_TIME)
    --DEBUG_TIME = 0
end

function BattleSearchSystem:IsCanSelect(entity,targetArgs)
...
function BattleSearchSystem:InRangeEntity(srcEntity,targetEntity,transInfo,range)
...
function BattleSearchSystem:GetPosByOfffset(posX,posZ,dirX,dirZ,offset)
...
function BattleSearchSystem:IsTargetTypeByTargetId(entity,targetUid,targetCondId)
...
function BattleSearchSystem:IsTargetType(entity,targetEntity,targetArgs)
...
function BattleSearchSystem:SelectEntity(entitys,entityDict,isCull)
    if #entitys <= 0 then
        return entitys
    end
