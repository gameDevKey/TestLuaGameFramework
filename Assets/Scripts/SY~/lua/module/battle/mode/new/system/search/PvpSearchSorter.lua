PvpSearchSorter = ExtendClass(BattlePvpSearchSystem)
--TODO:检查排序的稳定

function PvpSearchSorter:InitSort()
	self.sortMappings = 
	{
...
function PvpSearchSorter:EntitySort(targets)
	...
	self.selfRoadIndex = self.searchParam.entity.AreaComponent and self.searchParam.entity.AreaComponent.roadIndex or BattleDefine.RoadIndex.none
...
function PvpSearchSorter:LastSortResult(a,b)
...
function PvpSearchSorter:CheckLastSortResult(flag,a,b)
	if flag then
		return true
	elseif flag == nil then
		...
		elseif flag == nil then
			...
			local flag = self:same_road_Action(a,b)
			if flag then
				return true
			elseif flag == nil then
				return false
...
function PvpSearchSorter:SortIndex(a,b)
...
function PvpSearchSorter:default(a,b)
...
--优先离自己最近距离单位（直线距离）
function PvpSearchSorter:min_to_self_dis(a,b)
	local flag = self:min_to_self_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:min_to_self_dis_Action(a,b)
	local curPosX,curPosZ = self.searchParam.transInfo.posX,self.searchParam.transInfo.posZ
...
--优先离自己最远距离单位（直线距离）
function PvpSearchSorter:max_to_self_dis(a,b)
	local flag = self:max_to_self_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:max_to_self_dis_Action(a,b)
	local curPosX,curPosZ = self.searchParam.transInfo.posX,self.searchParam.transInfo.posZ
...
--离自己主堡最近
function PvpSearchSorter:min_to_self_home_dis(a,b)
	local flag = self:min_to_self_home_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:min_to_self_home_dis_Action(a,b)
...
--离自己主堡最远
function PvpSearchSorter:max_to_self_home_dis(a,b)
	local flag = self:max_to_self_home_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:max_to_self_home_dis_Action(a,b)
	local camp = self.searchParam.entity.CampComponent:GetCamp()
	local homeUid = self.world.BattleDataSystem:GetHomeUid(camp)
	local homeEntity = self.world.EntitySystem:GetEntity(homeUid)
	local homePos = homeEntity.TransformComponent:GetPos()
...
--离敌方堡最近
function PvpSearchSorter:min_to_enemy_home_dis(a,b)
	local flag = self:min_to_enemy_home_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:min_to_enemy_home_dis_Action(a,b)
...
--相同道路
function PvpSearchSorter:same_road(a,b)
	local flag = self:same_road_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:same_road_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)

	if not aEntity.AreaComponent or not bEntity.AreaComponent 
		or aEntity.AreaComponent.roadIndex == BattleDefine.RoadIndex.none 
		or bEntity.AreaComponent.roadIndex == BattleDefine.RoadIndex.none
		or self.selfRoadIndex == BattleDefine.RoadIndex.none then
		return nil
	elseif aEntity.AreaComponent.roadIndex == self.selfRoadIndex and bEntity.AreaComponent.roadIndex ~= self.selfRoadIndex then
		return true
	elseif aEntity.AreaComponent.roadIndex ~= self.selfRoadIndex and bEntity.AreaComponent.roadIndex == self.selfRoadIndex then
		return nil
	else
...
--离敌方堡最远
function PvpSearchSorter:max_to_enemy_home_dis(a,b)
	local flag = self:max_to_enemy_home_dis_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:max_to_enemy_home_dis_Action(a,b)
	local camp = self.searchParam.entity.CampComponent:GetEnemyCamp()
	local homeUid = self.world.BattleDataSystem:GetHomeUid(camp)
	local homeEntity = self.world.EntitySystem:GetEntity(homeUid)
	local homePos = homeEntity.TransformComponent:GetPos()
...
--地面单位
function PvpSearchSorter:floor_walk_unit(a,b)
	local flag = self:floor_walk_unit_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:floor_walk_unit_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--飞行单位
function PvpSearchSorter:fly_walk_unit(a,b)
	local flag = self:fly_walk_unit_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:fly_walk_unit_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量低
function PvpSearchSorter:hp_low(a,b)
	local flag = self:hp_low_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:hp_low_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量高
function PvpSearchSorter:hp_height(a,b)
	local flag = self:hp_height_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:hp_height_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量比例低
function PvpSearchSorter:hp_ratio_low(a,b)
	local flag = self:hp_ratio_low_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:hp_ratio_low_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量比例高
function PvpSearchSorter:hp_ratio_height(a,b)
	local flag = self:hp_ratio_height_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:hp_ratio_height_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量上限低
function PvpSearchSorter:max_hp_low(a,b)
	local flag = self:max_hp_low_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:max_hp_low_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--血量上限高
function PvpSearchSorter:max_hp_height(a,b)
	local flag = self:max_hp_height_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:max_hp_height_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--攻击低
function PvpSearchSorter:atk_low(a,b)
	local flag = self:atk_low_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:atk_low_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
...
--攻击低
function PvpSearchSorter:atk_height(a,b)
	local flag = self:atk_height_Action(a,b)
	return self:CheckLastSortResult(flag,a,b)
end

function PvpSearchSorter:atk_height_Action(a,b)
	local aEntity = self.world.EntitySystem:GetEntity(a)
	local bEntity = self.world.EntitySystem:GetEntity(b)
