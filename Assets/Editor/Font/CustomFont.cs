...
public class CustomFont
{
    ...
    [MenuItem("工具库/创建自定义字体", false, 100)]
    static private void CreateCustomFont()
    {
        ...
        var setting = ProjectSetting.GetData();
        ...
        //if (!folderPath.StartsWith(setting.FontPath) || !IOUtils.ExistFolder(absPath))
        //{
        //    Debug.LogErrorFormat("路径错误[{0}]（应该是 {1} 下面的文件夹）", folderPath, artPath);
        //    return;
        //}

        Debug.Log("字体资源路径:" + folderPath);

        string folderName = FileUtils.GetLastName(folderPath);
